...
expr     :    '(' expr ')'     {  $$ = $2;   }
         |    '|' expr '|'     {  $$ = newQuaternion(norm($2),0,0,0);   }
         |    '-' expr    {  $$ = scalarTimesQuaternion(-1,$2);  }
         |    expr '+' expr    {  $$ = sum($1,$3);  }
         |    expr '-' expr    {  $$ = diff($1,$3);  }
         |    expr '*' expr    {  $$ = product($1,$3);  }
         |    expr '/' expr    {  $$ = quotient($1,$3);  }
         |    expr '^' num    {  $$ = power($1,$3);  }
         |    expr '^' '-' num    {  $$ = inverse(power($1,$4));  }
         |    ROTATION '(' num ',' expr ')'    {  $$ = rotation($3,$5);  }
         |    num    {  $$ = newQuaternion($1,0,0,0);  }
         |    'i'    {  $$ = newQuaternion(0,1,0,0);   }
         |    'j'    {  $$ = newQuaternion(0,0,1,0);   }
         |    'k'    {  $$ = newQuaternion(0,0,0,1);   }
         |    num 'i'    {  $$ = newQuaternion(0,$1,0,0);   }
         |    num 'j'    {  $$ = newQuaternion(0,0,$1,0);   }
         |    num 'k'    {  $$ = newQuaternion(0,0,0,$1);   }
         ;
